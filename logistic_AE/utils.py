...
def slice_graph_predictions(mat_adj,mat_logits_0,mat_logits_1, graph_size, start_point):
    """
    ...
    graph_logits_adj = torch.squeeze(mat_adj[start_point:start_point + graph_size]) 
...
def reconstruction_accuracy(mat_logits_adj,mat_logits_0,mat_logits_1 , edge_index, batch_index,discrete_edge_att):
    ...
    #adj
    mat_discrete_adj = torch.squeeze(torch.tensor(torch.sigmoid(mat_logits_adj) > 0.5, dtype=torch.int32))
    ...
    return acc_adj, acc_0,acc_1, num_recon_0, num_recon_1, num_recon_adj
...
def slice_edge_type_from_edge_feats(edge_feats):
    ...
    rappresentation
    """
    edge_types_one_hot = edge_feats[:, :2]
