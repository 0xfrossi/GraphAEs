...
class GVAE(nn.Module):
    def __init__(self, feature_size):
        ...
        encoder_embedding_size = 27
        self.latent_embedding_size = 13
        decoder_size = 90
        edge_dim = 3
        hidden_size=20
        ...
        # Decoder layers
        self.decoder_dense_1 = Linear(self.latent_embedding_size*2, 52)
        self.decoder_bn_1 = BatchNorm1d(52)
        self.decoder_dense_2 = Linear(52, 52)
        self.decoder_bn_2 = BatchNorm1d(52)


        # mat first edge type at1
        self.decoder_dense_22 = Linear(52, decoder_size)
        self.decoder_bn_22 = BatchNorm1d(decoder_size)

      
        #mat second edge type at2
        self.decoder_d_first_mat_2= Linear(52, 64)
        self.decoder_bn_first_mat_2=BatchNorm1d(64)
        self.decoder_d_3= Linear(64, decoder_size)
        self.decoder_bn_3 = BatchNorm1d(decoder_size)
        self.decoder_drop_mat_2=nn.Dropout(0.2)
        self.decoder_reduce_mat_2=Linear(decoder_size, 45)
        self.decoder_bn_reduce_mat_2 = BatchNorm1d(45)
        self.decoder_last_mat_2=Linear(45, 32)
        self.decoder_bn_last_mat_2 = BatchNorm1d(32)

        self.decoder_last_mat_2_2=Linear(32, 16)
        self.decoder_bn_last_mat_2_2 = BatchNorm1d(16)
        self.decoder_last_mat_2_3=Linear(16, 8)
        self.decoder_bn_last_mat_2_3 = BatchNorm1d(8)

        # mat 3th edge type empty1
        self.decoder_d_first_mat_3= Linear(52, 64)
        self.decoder_bn_first_mat_3=BatchNorm1d(64)
        self.decoder_d_mat_3= Linear(64, decoder_size)
        self.decoder_bn_mat_3=BatchNorm1d(decoder_size)
        self.decoder_reduce_mat_3=Linear(decoder_size, 45)
        self.decoder_bn_reduce_mat_3 = BatchNorm1d(45)
        self.decoder_last_mat_3=Linear(45, 32)
        self.decoder_bn_last_mat_3 = BatchNorm1d(32)

        #exits
        self.decoder_dense_4 = Linear(decoder_size, 1)
        self.decoder_dense_5 = Linear(8, 1)
        self.decoder_dense_mat_3=Linear(32, 1)
    ...
    def decode(self, z, batch_index):
        ...
        for graph_id in torch.unique(batch_index):
            ...
            # Get indices for adjacency matrix, graph_z.shape[0]= num nodi
        ...
        x3= self.decoder_d_first_mat_3(x).relu()
        x3= self.decoder_bn_first_mat_3(x3)
        x3= self.decoder_d_mat_3(x3).relu()
        x3 = self.decoder_bn_mat_3(x3)
        x3= self.decoder_reduce_mat_3(x3).relu()
        x3= self.decoder_bn_reduce_mat_3(x3)
        x3= self.decoder_last_mat_3(x3).relu()
        x3=self.decoder_bn_last_mat_3(x3)
        edge_logits3= self.decoder_dense_mat_3(x3)
        ...
        x2=self.decoder_d_first_mat_2(x).relu()
        x2=self.decoder_bn_first_mat_2(x2)
        x2=self.decoder_d_3(x2).relu()
        x2=self.decoder_bn_3(x2)
        x2=self.decoder_drop_mat_2(x2)
        x2=self.decoder_reduce_mat_2(x2).relu()
        x2=self.decoder_bn_reduce_mat_2(x2)
        x2=self.decoder_last_mat_2(x2).relu()
        x2=self.decoder_bn_last_mat_2(x2)
        x2=self.decoder_last_mat_2_2(x2).relu()
        x2=self.decoder_bn_last_mat_2_2(x2)
        x2=self.decoder_last_mat_2_3(x2).relu()
        x2=self.decoder_bn_last_mat_2_3(x2)
        edge_logits2 = self.decoder_dense_5(x2)


        #[_, 1]
        
        return edge_logits1,edge_logits2,edge_logits3
    ...
    def forward(self, x, edge_attr, edge_index, batch_index): #batch_index: array(0,0,0,0.....31,31,31) è batch.batch
        ...
        mat_logits1,mat_logits2,mat_logits3 = self.decode(z, batch_index) #x=935, logits[17217, 1]---- x=1025 logits[22101, 1]

        #return mat_logits_adj,mat_logits1, mat_logits2
        return mat_logits1, mat_logits2,mat_logits3